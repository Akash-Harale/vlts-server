...
POST  
http://localhost:3005/api/assignroute
...
exports.fetchTrips = async (req, res) => {
  ...
  try {
    const { page = 1, limit = 10, vehicleReg } = req.query;
    ...
    if (vehicleReg) {
      query["vehicle_id.registration_number"] = {
        $regex: vehicleReg,
        $options: "i",
      };
    }
...
exports.fetchTripsByVehicle = async (req, res) => {
  console.log("tripController: fetchTripsByVehicle API: req.query: ", req.query);
...
// Fetch all assigned routes (trips) by trip_approval_status = APPROVED/PENDING/CANCELLED
...
exports.fetchTripsByStatus = async (req, res) => {
  ...
  try {
    ...
    console.log('routeAssignmentController: fetchAssignedRoutes by Trip_APPROVAL_STATUS: ', assignments);
    ...
    const vehicleStates = await VehicleState.find({ vehicle_id: { $in: vehicleIds } }).session(session);
...
exports.updateTripStatus = async (req, res) => {
  console.log("tripController: updateTripStatus API called: req.body:", req.body);
  ...
  if (!assignment_id) {
    return res.status(400).json({
      success: false,
      message: "Provide assignment_id in req.body to update!"
    });
  }
