...
const PORT = 5000;
const RETRY_TIME = 1000; // 30 seconds
...
function generateTelemetry(device) {
    return {
        imei: device.imei,
        lat: device.lat + (Math.random() - 0.5) * 1.11,
        lon: device.lon + (Math.random() - 0.5) * 1.01,
...
function simulateDevice(device) {
    ...
    const connect = () => {
        ...
        client.on('close', () => {
            console.log(`Device ${device.imei} disconnected. Retrying in 30 seconds...`);
